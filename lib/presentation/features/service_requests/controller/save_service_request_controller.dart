...
class SaveServiceRequestController extends GetxController{
  ...
  SaveServiceRequestController({@required this.repository}) : assert(repository != null);
  final _saveRequestData = SrComplaintModel().obs;
  get saveRequestData => _saveRequestData.value;
  set saveRequestData(value) =>
      _saveRequestData.value = value;
  ...
  getAccessKeyAndSaveRequest(List<File> imageList, SaveServiceRequest saveRequestModel) {
    ...
    repository.getAccessKey().then((data) async{
      ...
      await _prefs.then((SharedPreferences prefs) async {
        ...
        saveServiceRequest(imageList, data.accessKey, userSecurityKey, saveRequestModel).then((value){
          print(value);
          Get.back();
  ...
  Future<Map> saveServiceRequest(List<File> imageList, String accessKey, String userSecurityKey,SaveServiceRequest saveRequestModel){
    return repository.saveServiceRequest(imageList, accessKey, userSecurityKey, saveRequestModel).whenComplete(() => responseForDialog ='Test');
  }
}